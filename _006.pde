...
size(6000, 4000, PDF, "exportPDF.pdf");
//size(6000, 4000);
background(0);
...
for (x = 1; x < xmax; x++) {
    for (y = 1; y < ymax; y++) {
        ...
        for (i = 0; i < points; i++) {
            ...
            line(int(thisline[0])+x*400, int(thisline[1])+y*400, int(nextline[0])+x*400, int(nextline[1])+y*400); 
        }
        stroke(255);
        for (i = 0; i < points; i++) {
            ...
            line(int(drawpoint1[0])+x*400, int(drawpoint1[1])+y*400, int(drawpoint2[0])+x*400, int(drawpoint2[1])+y*400);
        }
    }
}
